...
- [x] `POST /auth/login` - login with JWT Bearer token, blocked until account activation
...
- [x] `POST /auth/reset-password` - validates email, DB-backed reset code hash, strong password rules, changes password, clears reset state, and activates inactive accounts
...
### Frontend (`apps/web/`)
- [x] `public/index.html` - landing page with Login/Register tabs, activation-aware registration/login messaging, forgot-password flow, reset code entry, and stronger password guidance
...
- [x] `public/login.html` - standalone login page with inactive-account activation hint
